...
for f in ['样本id','B14']:
    for f1 in categorical_columns:
        ...
        if cate_rate < 0.90:
            for f2 in li:
                col_name = f+'_to_' + f1 + "_" + f2 + '_mean'
                mean_columns.append(col_name)
                order_label = train.groupby([f1])[f2].mean()
                train[col_name] = train[f].map(order_label)
                miss_rate = train[col_name].isnull().sum() * 100 / train[col_name].shape[0]
                if miss_rate > 0:
                    train = train.drop([col_name], axis=1)
                    mean_columns.remove(col_name)
                else:
                    test[col_name] = test[f].map(order_label)
...
n=int((feature_importance.importance > 37).value_counts()[1])*(-1)
print(n,type(n))
...
feature_importance = feature_importance[feature_importance['score'] > 127]
